...
from transformers.generation_utils import calc_banned_ngram_tokens, calc_banned_bad_words_ids, \
...
class PretrainedBartModel(PreTrainedModel):
    ...
    def postprocess_next_token_scores(
        ...
        if bad_words_ids is not None:
            flat_bad_ids = [item for line in bad_words_ids for item in line]
            one_hot_ids = [0 for _ in range(250027)]
            for idx in flat_bad_ids:
                one_hot_ids[idx] = 1
            bad_ids_mask = torch.BoolTensor(one_hot_ids).to(scores.device)
            scores.masked_fill_(bad_ids_mask, -float("inf"))
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        all_encoder_attns = () if output_attentions else None
        next_decoder_cache = []
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_encoder_attn, layer_past = decoder_layer(
                x,
                encoder_hidden_states,
            ...
            if output_attentions:
                all_encoder_attns += (layer_encoder_attn,)
